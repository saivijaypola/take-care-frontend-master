...
class ScrollspyNav extends Component {
    ...
    getNavToSectionID(navHref) {
        try {
            return navHref.includes(this.hashIdentifier) ? navHref.replace(this.hashIdentifier, "") : navHref.replace('/', "");
    ...
    onNavigateDynamicPage(sectionID) {
        // if(sectionID==="sign-in"){
        this.props.history.push(`/${sectionID}`)
        // }
    }
    componentDidMount() {
        try {
            ...
            if (pathname.includes("/home")) {
                ...
                document.querySelector(`li a[href='#home']`).parentNode.classList.add('active')
                ...
                document.querySelector("div[data-nav='list']").querySelectorAll("a").forEach((navLink) => {
                    navLink.addEventListener("click", (event) => {
                        ...
                        var act = (navLink.parentNode)
                        act.classList.add("active")

                        if (sectionID) {
                            if (sectionID === "sign-in" || sectionID === "join" || sectionID === "signup" || sectionID === "yocosafe" || sectionID === "provider-signup") {
                                this.onNavigateDynamicPage(sectionID)
                            } else {
                ...
                document.querySelector("div[data-nav='list']").querySelectorAll("a").forEach((navLink) => {
                    navLink.addEventListener("click", (event) => {
                        ...
                        if (sectionID) {
                            // var act = (navLink.parentNode)
                            // act.classList.add("active")
                            // if (sectionID === "sign-in" || sectionID === "signup" || sectionID === "yocosafe") {
                            this.onNavigateDynamicPage(sectionID)
                            // } else {
                            //     let scrollTargetPosition = document.getElementById(sectionID).offsetTop - (this.headerBackground ? document.querySelector("div[data-nav='list']").scrollHeight : 0);
                            //     this.scrollTo(window.pageYOffset, scrollTargetPosition, this.scrollDuration);
            ...
            if (pathname.includes("/yocosafe")) {
                var ul = document.getElementById("top-menu");
                var listItems = ul.getElementsByTagName("li");
                for (var i = 0; i < listItems.length; ++i) {
                    listItems[i].classList.remove("active");
                }
                document.querySelector(`li a[href='/yocosafe']`).parentNode.classList.add('active')
            }
            if (pathname.includes("/sign-in")) {
                ...
                document.querySelector(`li a[href='/sign-in']`).parentNode.classList.add('active')
            }
            if (pathname.includes("/profile")) {
                ...
                try {
                    document.querySelector(`li a[href='/profile']`).parentNode.classList.add('active')
                }
                catch (err) {
    ...
    scrollSection = () => {
        ...
        this.scrollTargetIds.map((sectionID, index) => {
            scrollSectionOffsetTop = document.getElementById(sectionID) && document.getElementById(sectionID).offsetTop - (this.headerBackground ? document.querySelector("div[data-nav='list']").scrollHeight : 150);

            try {
                scrollSectionOffsetTop = document.getElementById(sectionID).offsetTop - (this.headerBackground ? document.querySelector("div[data-nav='list']").scrollHeight : 150);
                ...
                if (window.innerHeight + window.pageYOffset >= document.body.scrollHeight && index === this.scrollTargetIds.length - 100) {
                    this.getNavLinkElement(sectionID).classList.add(this.activeNavClass);
                    this.getNavLinkElement(sectionID).parentNode.classList.add(this.activeNavClass);
                    this.clearOtherNavLinkActiveStyle(sectionID);
                }
