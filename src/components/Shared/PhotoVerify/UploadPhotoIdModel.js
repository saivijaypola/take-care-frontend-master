...
class UploadPhotoIdModel extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const { isDocumentUploadDone, isUploading, isPhotoIdUpdated } = this.props;
        if (prevProps.isDocumentUploadDone !== isDocumentUploadDone && prevProps.isUploading !== isUploading) {
            if (isDocumentUploadDone && !isUploading && isPhotoIdUpdated) {
                        ...
                        UploadService.upload(currentFile, {
                            ...
                            if (response.data.data && response.data.data) {
                                ...
                                that.setState({
                                    ...
                                    userId: user.uid,
                                    uploadprogress: 0,
                                    isPhotoIdVerified: response.data.data.isVerified
                                }, () => {
    ...
    render() {
        ...
        return (
            <Modal isOpen={isModelOpen} role="dialog" autoFocus={true} centered={true}>
                <ModalHeader toggle={this.onToggleMode}>Upload Photo ID</ModalHeader>
                <ModalBody>
...
function mapStateToProps(state) {
    return {
        ...
        documentName: state.profile.documentName,
        isUploading: state.profile.isUploading,
        userDetails: state.profile.userDetails,
        isPhotoIdUpdated: state.profile.isPhotoIdUpdated,
    }
}
export default connect(mapStateToProps, { ...ProfileAction, ...DocumentsActions })(UploadPhotoIdModel)
